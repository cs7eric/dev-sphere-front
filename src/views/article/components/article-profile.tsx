...
import {selectUserInfo} from "@/store/features/user/userInfoSlice.ts";
...
const ArticleProfile: React.FC<Props> = ({article}) => {
  ...
  // 正确使用useSelector获取用户信息
  const userInfo = useSelector(selectUserInfo).userInfo;
  console.log('Redux中的用户信息:', userInfo);
      ...
      <section className="right col-span-1 space-y-4 relative">
        ...
        <div className="flex w-full absolute border-t pt-3 bottom-1 left-0 right-0 max-w-sm items-center space-x-2">
          <Image
            ...
            width={'36px'}
            height={'36px'}
            className={'rounded-md bg-neutral-700/20 p-0.5'}
          ></Image>
